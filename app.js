...
const firebaseConfig = {
  apiKey: "AIzaSyAGc4_kzXIDeZfyWwS_ovSLYXponmvxqIg",
  authDomain: "viveralmeidafutsal.firebaseapp.com",
  projectId: "viveralmeidafutsal",
  storageBucket: "viveralmeidafutsal.firebasestorage.app",
  messagingSenderId: "785840697300",
  appId: "1:785840697300:web:2d14a58f9678dcb6ad7041",
  measurementId: "G-L6MNDZY08B"
};
...
function defaultData(){
  return {
    ...
    groups: {A: [], B: []},
    matches: [],
    knockout: {qf: [], sf: [], final: [], champion: ""},
    createdAt: null
  };
}

function remoteDoc(){
  return db.collection("torneios").doc("viverAlmeida2026");
}

async function saveRemoteData(data){
  if(!remoteReady || applyingRemoteData) return;
  try {
    await remoteDoc().set({
      data: data,
    ...
    console.warn("Erro ao guardar no Firebase:", e);
  }
}
...
function startRealtimeSync(){
  ...
  remoteDoc().onSnapshot((doc) => {
    const remote = doc.exists ? doc.data().data : null;
    ...
    if(remote){
      ...
      if(typeof renderAdmin === "function" && document.getElementById("teamInputs")) renderAdmin();
      else if(typeof renderPublic === "function") renderPublic();
    } else {
      const local = JSON.parse(localStorage.getItem(STORAGE_KEY) || "null");
      if(local) saveRemoteData(local);
    }
  }, (error) => {
    console.warn("Erro no realtime:", error);
  });
}

window.addEventListener("load", () => {
  setTimeout(startRealtimeSync, 300);
});
...
function exportLocalData(){
  ...
  a.href = url;
  a.download = "dados-torneio-viver-almeida.json";
  a.click();
...
function syncLocalToFirebase(){
  const data = loadData();
  saveRemoteData(data);
  alert("Dados enviados para Firebase. Agora o público passa a receber as atualizações.");
}
