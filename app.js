...
function resultRow(m){
  return `<div class="admin-row">
    <strong>${m.teamA || "A definir"}</strong>
    <input type="number" min="0" id="ga-${m.id}" value="${m.goalsA}" placeholder="0">
    <input type="number" min="0" id="gb-${m.id}" value="${m.goalsB}" placeholder="0">
    <strong>${m.teamB || "A definir"}</strong>
    <button onclick="saveResult('${m.id}')">Guardar</button>
    ...
    <input style="grid-column:1/-1" type="datetime-local" id="dt-${m.id}" value="${m.datetime || ""}" onchange="saveDateTime('${m.id}')" title="Data e hora do jogo">
    <small style="grid-column:1/-1;color:#b8c7c0">${m.type === "knockout" ? m.round : "Grupo " + m.group} · Ao subir/descer, a hora muda automaticamente pela ordem</small>
  </div>`;
}
